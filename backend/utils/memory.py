...
MEMORY_DIR = os.path.join(PROJECT_ROOT, "data")
...
def _sanitize_memory(data):
    ...
    return {
        ...
        "scenarios": _sanitize_scenarios(data.get("scenarios", {})),
    }


def _sanitize_scenarios(data):
    if not isinstance(data, dict):
        return {}

    sanitized = {}
    for scenario_name, apps in data.items():
        normalized_scenario = normalize_text(scenario_name)
        if not normalized_scenario or not isinstance(apps, list):
            continue
        ...
        for app_name in apps:
            ...
            if not normalized_app or normalized_app in seen:
                continue
            seen.add(normalized_app)
            unique_apps.append(normalized_app)

        if unique_apps:
            sanitized[normalized_scenario] = unique_apps
...
def get_scenario_apps(scenario_name):
    normalized_scenario = normalize_text(scenario_name)
    ...
    with _memory_lock:
        memory = _get_memory()
        return list(memory.get("scenarios", {}).get(normalized_scenario, []))


def remember_scenario_app(scenario_name, app_name):
    normalized_scenario = normalize_text(scenario_name)
    normalized_app = normalize_text(app_name)
    if not normalized_scenario or not normalized_app:
        return False

    with _memory_lock:
        memory = _get_memory()
        scenarios = memory.setdefault("scenarios", {})
        apps = scenarios.setdefault(normalized_scenario, [])
        if normalized_app in apps:
            return False
